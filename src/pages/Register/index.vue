<template>
  <div class="register-container">
    ...
    <div class="register">
      ...
      <div class="controls">
        <input name="m1" type="checkbox" v-model="agreeChecked" />
        <span>同意协议并注册《尚品汇用户协议》</span>
        <span class="error-msg">错误提示信息</span>
      </div>
      <div class="btn">
        <button @click.prevent="registerUser">完成注册</button>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    //test phone: 13858764523
    async registerUser() {
      if (
        this.checkedPassword &&
        this.phoneNumber.length == 11 &&
        this.agreeChecked &&
        this.registerCode.length == 6
      ) {
        try {
          await this.$store.dispatch("user/registerUser", {
            phone: this.phoneNumber,
            password: this.checkedPassword,
